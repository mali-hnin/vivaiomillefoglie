class VivaioEventsController < ApplicationController
  ...
  def index
    if params[:scope] && params[:scope] == "past"
      @vivaio_events = VivaioEvent.past
    else
      @vivaio_events = VivaioEvent.by_event_date.future
    end
  end
  ...
  def vivaio_event_params
    params.require(:vivaio_event).permit(:title, :content, :data_evento, :data_fine_evento)
  end
end
